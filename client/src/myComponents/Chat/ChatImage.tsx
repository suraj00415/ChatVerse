...
const ChatSimple = function ({ isForwarded, message, isSender, isGroup, senderName, timeAgo, newDate, isPrevSender, color, messageData, replyToContent, replyToColor, replyToName, isReply }) {
    ...
    const images = ["https://res.cloudinary.com/dfu2zsjpw/image/upload/v1713140426/thinnvlh2yjbezpxsv6l.jpg", "http://res.cloudinary.com/dfu2zsjpw/image/upload/v1713983080/chofsctsqhkvgmvkdyrq.jpg", "https://res.cloudinary.com/dfu2zsjpw/image/upload/v1713139977/fvpncecgm1opx9fxbv4r.jpg", "http://res.cloudinary.com/dfu2zsjpw/image/upload/v1714133337/vwd4pfxldayk1tonwjv1.jpg"]
    return (
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            {images.map((item, index) => (
                                <PhotoView key={index} src={item}>
                                    {index < 4 ? <img style={{ objectFit: 'cover' }} className="cursor-pointer hover:opacity-60 h-36 w-auto" src={item} alt="" /> : undefined}
                                </PhotoView>
                            ))}
                        </div>
                    </PhotoProvider>
